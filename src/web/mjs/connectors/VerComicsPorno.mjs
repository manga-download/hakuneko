...
export default class VerComicsPorno extends Connector {
    constructor() {
        ...
        this.listPages = ".comicimg *";
    }

    async _getMangasFromPage(page) {
        let request = new Request(
            this.url + this.library + page,
            this.requestOptions
        );
    ...
    async _getPages(chapter) {
        ...
        let images = data.filter((el) => el.src);
        console.log(images);
        return images.map((element) =>
            this.getAbsolutePath(element.dataset["src"] || element, request.url)
        );
    }
}
