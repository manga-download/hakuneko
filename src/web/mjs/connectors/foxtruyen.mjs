...
export default class FoxTruyen extends Connector {
    constructor() {
        super();
        super.id = 'foxtruyen';
        super.label = 'FoxTruyen';
        this.tags = ['manga', 'webtoon', 'vietnamese'];
        this.url = 'https://foxtruyen.com';
    ...
    async _getMangas() {
        ...
        const request = new Request(uri, this.requestOptions);
        const data = await this.fetchDOM(request, 'div.pagination.mt-4.mb20 a:last-child');
    ...
    async _getMangasFromPage(page) {
        ...
        const request = new Request(uri, this.requestOptions);
        const data = await this.fetchDOM(request, 'div.row.list_item_home div.item_home a.book_name');
    ...
    async _getPages(chapter) {
        ...
        const request = new Request(uri, this.requestOptions);
        const data = await this.fetchDOM(request, 'div.content_detail source.lazy');
